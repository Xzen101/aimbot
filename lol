--// GODLOCK v20 - Split Loops: RenderStepped Aimbot, Heartbeat ESP (ESP 1000 Range, Lerp 0.35)
...
local Settings = {
    ...
    ESP = false,
    FOV = 200,
    ESPRange = 1000,  -- Set to 1000
    LockPart = "Head",
    TeamCheckAimbot = true,
    ESPTeamCheck = true,
    AimYOffset = 0,
    AimLerp = 0.35,  -- Changed to 0.35
}
...
local function getLockPart(char)
    return char[Settings.LockPart] or char.Head or char.UpperTorso
end
...
title.Text = "GODLOCK v20 - Split Loops"
...
--// AIMBOT UPDATE (RenderStepped for smoothness)
local function updateAimbot(dt)
    ...
    -- Update target every few frames
    if tick() - lastESPUpdate > 0.05 then
...
--// ESP UPDATE (Heartbeat for efficiency)
local function updateESP(dt)
    local cam = Camera
    if not cam then return end

    -- ESP (update slower)
    if Settings.ESP and tick() - lastESPUpdate > 0.1 then
...
print("GODLOCK v20 LOADED - Split Loops: RenderStepped Aimbot, Heartbeat ESP, ESP 1000 Range, Lerp 0.35")
